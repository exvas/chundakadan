...
def get_allocated_leaves(from_date, to_date, employee, leave_type):
	# only consider allocations in the first month of the filter period
	end_of_first_month = get_last_day(getdate(from_date))
	if getdate(to_date) > end_of_first_month:
		to_date = end_of_first_month
	...
	allocated_leaves = (
		...
		.where(
			...
			& (ledger.from_date[from_date:to_date]) # Changed to only check from_date
