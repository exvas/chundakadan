...
frappe.provide('chundakadan.utils');

chundakadan.utils.setup_item_selection = function(frm, child_table_name, is_purchase = false) {
    const child_doctype = frm.fields_dict[child_table_name].grid.doctype;
    ...
    frappe.ui.form.on(child_doctype, {
        item_code: function(frm, cdt, cdn) {
            ...
            if (row.item_code && !row._from_dialog) {
                ...
                chundakadan.utils.show_item_selection_dialog(frm, cdt, cdn, child_table_name, is_purchase);
    ...
    // Also style the item_code input
    if (frm.fields_dict[child_table_name].grid.get_field('item_code').$input) {
        frm.fields_dict[child_table_name].grid.get_field('item_code').$input.css({
            'cursor': 'pointer',
...
chundakadan.utils.show_item_selection_dialog = function(frm, cdt, cdn, child_table_name, is_purchase) {
    let dialog = new frappe.ui.Dialog({
        ...
        fields: [
            ...
            },
            {
                ...
                click: function() {
                    chundakadan.utils.search_filtered_items(dialog, frm, cdt, cdn, child_table_name, is_purchase);
                }
            },
            {
            ...
            frappe.call({
                ...
                callback: function(r) {
                    if (r.message && r.message.name) {
                        chundakadan.utils.select_item_from_dialog(r.message.name, cdt, cdn, child_table_name);
                        dialog.hide();
                    } else {
...
chundakadan.utils.search_filtered_items = function(dialog, frm, cdt, cdn, child_table_name, is_purchase) {
    let item_group = dialog.get_value('item_group');
    let finish = dialog.get_value('finish');
    let size = dialog.get_value('size');
    ...
    frappe.call({
        ...
        callback: function(r) {
            if (r.message && r.message.length > 0) {
                chundakadan.utils.display_items_grid(dialog, r.message, frm, cdt, cdn, child_table_name);
            } else {
                dialog.fields_dict.items_html.$wrapper.html(`
...
chundakadan.utils.display_items_grid = function(dialog, items, frm, cdt, cdn, child_table_name) {
    let html = `
    ...
    items.forEach(item => {
        ...
        html += `
            <div class="item-card" onclick="chundakadan.utils.select_item_from_dialog('${item.name}', '${cdt}', '${cdn}', '${child_table_name}')">
...
chundakadan.utils.select_item_from_dialog = function(item_code, cdt, cdn, child_table_name) {
    let row = locals[cdt][cdn];
    row._from_dialog = true;

    frappe.model.set_value(cdt, cdn, 'item_code', item_code).then(() => {
        let grid_row = cur_frm.fields_dict[child_table_name].grid.grid_rows_by_docname[cdn];
        if (grid_row) {
